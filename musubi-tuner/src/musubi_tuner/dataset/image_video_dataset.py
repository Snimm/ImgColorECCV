...
class ImageJsonlDatasource(ImageDatasource):
    def __init__(self, image_jsonl_file: str, control_count_per_image: Optional[int] = None, multiple_target: bool = False, auto_grayscale_control: bool = False):
        super().__init__()
        self.image_jsonl_file = image_jsonl_file
        self.control_count_per_image = control_count_per_image
        self.multiple_target = multiple_target
        self.auto_grayscale_control = auto_grayscale_control
        self.current_idx = 0
        ...
        if self.auto_grayscale_control:
            self.has_control = True
            logger.info("Auto grayscale control enabled")
    ...
    def get_image_data(self, idx: int) -> tuple[str, list[Image.Image], str, Optional[list[Image.Image]]]:
        ...
        if self.auto_grayscale_control:
            if controls is None:
                controls = []
            # Generate grayscale control images from current images
            for img in images:
                # Convert to L (grayscale) then back to RGB to keep 3 channels
                control = img.convert("L").convert("RGB")
                controls.append(control)
...
class ImageDataset(BaseDataset):
    def __init__(
        ...
        debug_dataset: bool = False,
        architecture: str = "no_default",
        auto_grayscale_control: bool = False,
    ):
        ...
        if image_directory is not None:
            ...
            self.datasource = ImageJsonlDatasource(image_jsonl_file, control_count_per_image, multiple_target, auto_grayscale_control=self.auto_grayscale_control)
        else:
    ...
    def prepare_for_training(self, num_timestep_buckets: Optional[int] = None):
        ...
        if self.cache_directory is not None:
            ...
            # Fallback for on-the-fly training (no cache)
            logger.info("Preparing dataset without cache (on-the-fly)...")
            
            # Assuming ImageJsonlDatasource has .data attribute
            if hasattr(self.datasource, 'data'):
                total_images = len(self.datasource.data)
                logger.info(f"Bucketing {total_images} images...")
                
                for i, item in enumerate(self.datasource.data):
                    # Datasource data item structure depends on datasource type
                    # ImageJsonlDatasource: item is dict from jsonl
                    image_path = item.get("image_path", item.get("image_path_0"))
                    caption = item.get("caption", "")
                    
                    if image_path and os.path.exists(image_path):
                        try:
                            # Lazy load to get size
                            with Image.open(image_path) as img:
                                image_size = img.size
                        except Exception as e:
                            logger.warning(f"Failed to open image for bucketing: {image_path}: {e}")
                            continue
                            
                        item_key = os.path.splitext(os.path.basename(image_path))[0]
                        bucket_reso = bucket_selector.get_bucket_resolution(image_size)

                        # Handle control if needed (Flux Klein 4B takes input image as control?)
                        # Typically auto_grayscale_control implies we generate control on fly.
                        # So control resolution matches image resolution.
                        if self.no_resize_control or self.control_resolution is not None:
                             # Should split bucket based on control resolution? 
                             # If control is generated from image, it matches image bucket.
                             pass

                        item_info = ItemInfo(item_key, caption, image_size, bucket_reso, latent_cache_path=None, image_path=image_path)
                        item_info.text_encoder_output_cache_path = None # On-the-fly TE encoding needed
                        ...
                        if i % 1000 == 0:
                            logger.info(f"Bucketed {i}/{total_images} images")
            else:
                logger.warning("Datasource does not support direct data access for on-the-fly bucketing.")
    ...
    def __getitem__(self, idx):
        ...
        batch_infos = self.batch_manager[idx]
        
        # If batch_infos is a dict, it means it's already a batch of tensors (cached), so return it directly
        if isinstance(batch_infos, dict):
            return batch_infos

        # Create copies with loaded content to avoid memory leak in BatchManager
        new_batch_infos = []
        for item_info in batch_infos:
            if item_info.content is None and item_info.image_path is not None:
                import copy
                new_item = copy.copy(item_info)
                try:
                    img = Image.open(new_item.image_path).convert("RGB")
                    
                    # Resize to bucket resolution
                    if new_item.bucket_size:
                        w, h = new_item.bucket_size[0], new_item.bucket_size[1]
                        image_np = resize_image_to_bucket(img, (w, h))
                    else:
                        image_np = np.array(img)

                    new_item.content = image_np
                    
                    if self.auto_grayscale_control:
                        # Create grayscale control
                        # image_np is RGB (uint8)
                        control = cv2.cvtColor(image_np, cv2.COLOR_RGB2GRAY)
                        control = control[..., np.newaxis] # (H, W, 1)
                        new_item.control_content = [control] # List for multiple controls
                        
                    new_batch_infos.append(new_item)
                except Exception as e:
                    logger.error(f"Error loading image {new_item.image_path}: {e}")
                    raise e
            else:
                new_batch_infos.append(item_info)
                
        return new_batch_infos
